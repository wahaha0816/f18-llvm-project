...
program nested_where
  ...
  ! CHECK: %[[VAL_254:.*]] = constant 3 : i64
  ! CHECK: %[[VAL_255:.*]] = fir.convert %[[VAL_254]] : (i64) -> index
  ! CHECK: %[[VAL_256:.*]] = constant 1 : index
  ! CHECK: %[[VAL_257:.*]] = constant 0 : index
  ! CHECK: %[[VAL_258:.*]] = subi %[[VAL_255]], %[[VAL_256]] : index
  ! CHECK: %[[VAL_259:.*]] = fir.do_loop %[[VAL_260:.*]] = %[[VAL_257]] to %[[VAL_258]] step %[[VAL_256]] unordered iter_args(%[[VAL_261:.*]] = %[[VAL_190]]) -> (!fir.array<3xi32>) {
  ! CHECK: %[[VAL_262:.*]] = constant 1 : index
  ! CHECK: %[[VAL_263:.*]] = addi %[[VAL_260]], %[[VAL_262]] : index
  ! CHECK: %[[VAL_264:.*]] = fir.array_coor %[[VAL_214]](%[[VAL_222]]) %[[VAL_263]] : (!fir.ref<!fir.array<?xi8>>, !fir.shape<1>, index) -> !fir.ref<i8>
  ! CHECK: %[[VAL_265:.*]] = fir.load %[[VAL_264]] : !fir.ref<i8>
  ! CHECK: %[[VAL_266:.*]] = fir.convert %[[VAL_265]] : (i8) -> i1
  ! CHECK: %[[VAL_267:.*]] = fir.if %[[VAL_266]] -> (!fir.array<3xi32>) {
  ! CHECK: %[[VAL_268:.*]] = constant 1 : index
  ! CHECK: %[[VAL_269:.*]] = addi %[[VAL_260]], %[[VAL_268]] : index
  ! CHECK: %[[VAL_270:.*]] = fir.array_coor %[[VAL_245]](%[[VAL_253]]) %[[VAL_269]] : (!fir.ref<!fir.array<?xi8>>, !fir.shape<1>, index) -> !fir.ref<i8>
  ...
  ! CHECK: %[[VAL_274:.*]] = constant 1 : i32
  ! CHECK: %[[VAL_275:.*]] = constant 1 : index
  ! CHECK: %[[VAL_276:.*]] = addi %[[VAL_260]], %[[VAL_275]] : index
  ...
  ! CHECK: } else {
  ! CHECK: fir.result %[[VAL_261]] : !fir.array<3xi32>
  ! CHECK: }
  ! CHECK: fir.result %[[VAL_278:.*]] : !fir.array<3xi32>
  ! CHECK: } else {
  ! CHECK: fir.result %[[VAL_261]] : !fir.array<3xi32>
  ! CHECK: }
  ! CHECK: fir.result %[[VAL_279:.*]] : !fir.array<3xi32>
  ! CHECK: }
  ! CHECK: fir.result %[[VAL_280:.*]] : !fir.array<3xi32>
  ! CHECK: }
  ! CHECK: fir.array_merge_store %[[VAL_35]], %[[VAL_281:.*]] to %[[VAL_5]] : !fir.array<3xi32>, !fir.array<3xi32>, !fir.ref<!fir.array<3xi32>>
  ! CHECK: %[[VAL_282:.*]] = fir.convert %[[VAL_3]] : (!fir.ref<tuple<i64, !fir.heap<!fir.array<?xi8>>, !fir.heap<!fir.array<?xi64>>>>) -> !fir.llvm_ptr<i8>
  ! CHECK: %[[VAL_283:.*]] = fir.call @_FortranARaggedArrayDeallocate(%[[VAL_282]]) : (!fir.llvm_ptr<i8>) -> none
  ! CHECK: %[[VAL_284:.*]] = fir.convert %[[VAL_4]] : (!fir.ref<tuple<i64, !fir.heap<!fir.array<?xi8>>, !fir.heap<!fir.array<?xi64>>>>) -> !fir.llvm_ptr<i8>
  ! CHECK: %[[VAL_285:.*]] = fir.call @_FortranARaggedArrayDeallocate(%[[VAL_284]]) : (!fir.llvm_ptr<i8>) -> none
  integer :: a(3) = 0
