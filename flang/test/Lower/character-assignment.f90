...
subroutine assign1(lhs, rhs)
  ...
  ! CHECK: %[[tmp_alloc:.*]] = fir.alloca !fir.char<1>, %[[min_len]]
  ! CHECK: %[[tmp:.*]] = fir.convert %[[tmp_alloc]] : (!fir.ref<!fir.char<1>>) -> !fir.ref<!fir.char<1,?>>
...
subroutine assign_constant(lhs)
  ...
  ! CHECK: %[[tmp_alloc:.*]] = fir.alloca !fir.char<1>, %{{.*}}
  ! CHECK: %[[tmp:.*]] = fir.convert %[[tmp_alloc]] : (!fir.ref<!fir.char<1>>) -> !fir.ref<!fir.char<1,?>>
  lhs = "Hello World"
  ...
  ! CHECK: fir.do_loop %[[j:.*]] = %{{.*}} to %{{.*}} {
    ! CHECK-DAG: %[[jhs_cast:.*]] = fir.convert %[[lhs]]#0
