...
! Test OPTIONAL lowering on caller/callee. Also test PRESENT intrinsic.
module opt
contains
...
subroutine call_intrinsic_scalar()
  ...
  ! CHECK: %[[absent:.*]] = fir.convert %c0{{.*}} : (index) -> !fir.ref<f32>
...
subroutine call_intrinsic_f77_array()
  ...
  ! CHECK: %[[absent:.*]] = fir.convert %c0{{.*}} : (index) -> !fir.ref<!fir.array<100xf32>>
...
subroutine call_character_scalar()
  ...
  character(10) :: x
  ! CHECK: %[[addrCast:.*]] = fir.convert %0
  ! CHECK: %[[x:.*]] = fir.emboxchar %[[addrCast]], {{.*}}
  ! CHECK: fir.call @_QMoptPcharacter_scalar(%[[x]]) : (!fir.boxchar<1>) -> ()
  call character_scalar(x)
  ! CHECK: %[[null:.*]] = fir.convert %c0 : (index) -> !fir.ref<!fir.char<1,?>>
  ! CHECK: %[[undef:.*]] = fir.undefined index
  ! CHECK: %[[absent:.*]] = fir.emboxchar %[[null]], %[[undef]] : (!fir.ref<!fir.char<1,?>>, index) -> !fir.boxchar<1>
...
subroutine call_assumed_shape()
  ...
  call assumed_shape()
  ! CHECK: %[[null:.*]] = fir.convert %c0{{.*}} : (index) -> !fir.ref<i32>
  ! CHECK: %[[nullBox:.*]] = fir.embox %[[null]] : (!fir.ref<i32>) -> !fir.box<i32>
  ! CHECK: %[[absent:.*]] = fir.convert %[[nullBox]] : (!fir.box<i32>) -> !fir.box<!fir.array<?xf32>>
...
subroutine call_allocatable_array()
  ...
  ! CHECK: %[[absent:.*]] = fir.convert %c0_1 : (index) -> !fir.ref<!fir.box<!fir.heap<!fir.array<?xf32>>>>
