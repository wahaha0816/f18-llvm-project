...
static mlir::CmpIOp createCondition(fir::FirOpBuilder &builder) {
  auto loc = builder.getUnknownLoc();
  ...
  return builder.create<mlir::CmpIOp>(
      loc, mlir::CmpIPredicate::eq, zero1, zero2);
}
...
TEST_F(FIRBuilderTest, genIsNotNull) {
  ...
  EXPECT_TRUE(mlir::isa<mlir::CmpIOp>(res.getDefiningOp()));
  auto cmpOp = dyn_cast<mlir::CmpIOp>(res.getDefiningOp());
  EXPECT_EQ(mlir::CmpIPredicate::ne, cmpOp.predicate());
}
...
TEST_F(FIRBuilderTest, genIsNull) {
  ...
  EXPECT_TRUE(mlir::isa<mlir::CmpIOp>(res.getDefiningOp()));
  auto cmpOp = dyn_cast<mlir::CmpIOp>(res.getDefiningOp());
  EXPECT_EQ(mlir::CmpIPredicate::eq, cmpOp.predicate());
}
...
TEST_F(FIRBuilderTest, createRealZeroConstant) {
  ...
  EXPECT_TRUE(mlir::isa<mlir::ConstantOp>(cst.getDefiningOp()));
  auto cstOp = dyn_cast<mlir::ConstantOp>(cst.getDefiningOp());
...
TEST_F(FIRBuilderTest, createGlobal1) {
  ...
  EXPECT_EQ(global.sym_name(), g1.sym_name());
  auto g2 = builder.getNamedGlobal("global7");
  EXPECT_TRUE(nullptr == g2);
  auto g3 = builder.getNamedGlobal("");
  EXPECT_TRUE(nullptr == g3);
}
...
TEST_F(FIRBuilderTest, locationToFilename) {
  ...
  auto symbol = addrOp.symbol().getRootReference();
