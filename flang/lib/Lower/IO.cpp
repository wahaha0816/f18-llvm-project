...
static void genInputItemList(Fortran::lower::AbstractConverter &converter,
  ...
  for (auto &item : items) {
    ...
    if (argType.isa<fir::BoxType>()) {
      ...
      if (charHelper.isCharacter(itemTy)) {
        auto len = charHelper.getLen(itemBox);
      ...
      ok = builder.create<fir::CallOp>(loc, inputFunc, inputFuncArgs)
               .getResult(0);
    }
  }
  ...
  static void genIoLoop(Fortran::lower::AbstractConverter & converter,
    ...
    for (auto *op = builder.getBlock()->getParentOp(); isa<fir::IfOp>(op);
      ...
      builder.create<fir::ResultOp>(loc,
                                    lastOp->getResult(0)); // runtime result
  ...
  static mlir::Value locationToFilename(Fortran::lower::AbstractConverter &
                                            converter,
  ...
  static mlir::Value getDefaultScratchLen(
  ...
  template <typename A>
  static std::tuple<mlir::Value, mlir::Value, mlir::Value> lowerStringLit(
  ...
  template <>
  mlir::Value genIOOption<Fortran::parser::Name>(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      mlir::Value cookie, const Fortran::parser::Name &spec) {
    // namelist
  ...
  template <typename A>
  static mlir::OpBuilder::InsertPoint threadSpecs(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
  ...
  /// Most I/O statements have some form of optional exception condition
  /// handling for when there is a failure. There are 5 basic forms: ERR, EOR,
  /// END, IOSTAT, and IOMSG. The first three cause control-flow to transfer to
  /// another statement. The final two return information from the runtime, via
  /// a variable, about the nature of I/O failure. Not all statements have all 5
  /// forms, but this handler will match the ones that do occur in the spec-list
  /// of a particular statement.
  template <typename A>
  static void genConditionHandlerCall(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      mlir::Value cookie, const A &specList, ConditionSpecInfo &csi) {
    for (const auto &spec : specList) {
      std::visit(
          Fortran::common::visitors{
              ...
              [&](const Fortran::parser::InquireSpec::IntVar &var) {
                if (std::get<Fortran::parser::InquireSpec::IntVar::Kind>(
                        var.t) ==
  ...
  /// returns true iff the expression in the parse tree is not really a format
  /// but rather a namelist variable.
  template <typename A>
  ...
  /// a select operation to allow for run-time selection of the format
  /// statement.
  static std::tuple<mlir::Value, mlir::Value, mlir::Value>
  lowerReferenceAsStringSelect(
      ...
      mlir::Type strTy, mlir::Type lenTy) {
    // Possible optimization TODO: Instead of inlining a selectOp every time
    // there is a variable reference to a format statement, a function with the
    // selectOp could be generated to reduce code size. It is not clear if such
    // an optimization would be deployed very often or improve the object code
    ...
    for (auto label : labels) {
      ...
      // Pass the format string reference and the string length out of the
      // select statement
    ...
    // Handle and return the string reference and length selected by the
    // selectOp
  ...
  /// label of a format statement, and the * case.  The first three are done
  /// here. The * case is done elsewhere.
  static std::tuple<mlir::Value, mlir::Value, mlir::Value> genFormat(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
  ...
  template <typename A>
  std::tuple<mlir::Value, mlir::Value, mlir::Value> getFormat(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
    ...
    return genFormat(converter, loc,
                     *getIOControl<Fortran::parser::Format>(stmt), strTy,
                     lenTy);
  }
  template <>
  ...
  /// Generate a reference to a buffer and the length of buffer. There are 3
  /// cases An IoUnit can be variable, a ScalarIntExpr (i.e FileUnitNumber) or a
  /// *. Only the first case (a variable) is handled here.
  static std::tuple<mlir::Value, mlir::Value> genBuffer(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
  ...
  template <typename A>
  std::tuple<mlir::Value, mlir::Value> getBuffer(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      const A &stmt, mlir::Type strTy, mlir::Type lenTy) {
    if (stmt.iounit)
      return genBuffer(converter, loc, *stmt.iounit, strTy, lenTy);
    return genBuffer(converter, loc,
                     *getIOControl<Fortran::parser::IoUnit>(stmt), strTy,
                     lenTy);
  }

  /// Create a boxed value to be passed to the runtime. `descRef` is the
  /// reference to an entity that is to be boxed. Array entities are boxed with
  /// a shape.
  static mlir::Value getDescriptor(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      const fir::ExtendedValue &descRef, mlir::Type toType) {
    auto &builder = converter.getFirOpBuilder();
    mlir::Value embox;
  ...
  static mlir::Value genIOUnit(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      const Fortran::parser::IoUnit &iounit, mlir::Type ty) {
    auto &builder = converter.getFirOpBuilder();
  ...
  mlir::Value getIOUnit(Fortran::lower::AbstractConverter & converter,
    ...
    return genIOUnit(converter, loc,
                     *getIOControl<Fortran::parser::IoUnit>(stmt), ty);
  }
  ...
  static mlir::Value genBasicIOStmt(
      Fortran::lower::AbstractConverter & converter, const S &stmt) {
    auto &builder = converter.getFirOpBuilder();
    auto loc = converter.getCurrentLocation();
    ...
    auto cookie =
        builder.create<fir::CallOp>(loc, beginFunc, args).getResult(0);
  ...
  mlir::FuncOp getBeginDataTransfer(
      mlir::Location loc, FirOpBuilder & builder, bool isFormatted, bool isList,
      bool isIntern, bool isOtherIntern, bool isAsynch, bool isNml) {
    if constexpr (isInput) {
      ...
      if (isFormatted) {
        if (isIntern) {
          if (isNml)
            return getIORuntimeFunc<mkIOKey(BeginInternalNamelistInput)>(
                loc, builder);
          if (isOtherIntern) {
            if (isList)
          ...
          if (isList)
            return getIORuntimeFunc<mkIOKey(BeginInternalListInput)>(loc,
                                                                     builder);
          return getIORuntimeFunc<mkIOKey(BeginInternalFormattedInput)>(
              loc, builder);
        }
        if (isNml)
          return getIORuntimeFunc<mkIOKey(BeginExternalNamelistInput)>(loc,
                                                                       builder);
        if (isList)
          return getIORuntimeFunc<mkIOKey(BeginExternalListInput)>(loc,
                                                                   builder);
        ...
        if (isIntern) {
          ...
          if (isList)
            return getIORuntimeFunc<mkIOKey(BeginInternalListOutput)>(loc,
                                                                      builder);
          return getIORuntimeFunc<mkIOKey(BeginInternalFormattedOutput)>(
              loc, builder);
        }
        if (isNml)
          return getIORuntimeFunc<mkIOKey(BeginExternalNamelistOutput)>(
              loc, builder);
        if (isList)
          return getIORuntimeFunc<mkIOKey(BeginExternalListOutput)>(loc,
                                                                    builder);
  ...
  void genBeginCallArguments(
      llvm::SmallVector<mlir::Value, 8> & ioArgs,
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      const A &stmt, mlir::FunctionType ioFuncTy, bool isFormatted, bool isList,
      bool isIntern, bool isOtherIntern, bool isAsynch, bool isNml,
  ...
  static mlir::Value genDataTransferStmt(
      Fortran::lower::AbstractConverter & converter, const A &stmt) {
    auto &builder = converter.getFirOpBuilder();
    auto loc = converter.getCurrentLocation();
    ...
    mlir::FuncOp ioFunc =
        getBeginDataTransfer<isInput>(loc, builder, isFormatted, isList,
                                      isIntern, isOtherIntern, isAsynch, isNml);
    mlir::FunctionType ioFuncTy = ioFunc.getType();
    ...
    genBeginCallArguments<hasIOCtrl>(ioArgs, converter, loc, stmt, ioFuncTy,
                                     isFormatted, isList, isIntern,
                                     isOtherIntern, isAsynch, isNml, descRef);
    ioArgs.push_back(
  ...
  void Fortran::lower::genPrintStatement(
      Fortran::lower::AbstractConverter & converter,
      const Fortran::parser::PrintStmt &stmt) {
    // PRINT does not take an io-control-spec. It only has a format specifier,
    // so it is a simplified case of WRITE.
  ...
  template <typename A>
  static mlir::Value genInquireSpec(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      mlir::Value cookie, mlir::Value idExpr, const A &var) {
    // default case: do nothing
    return {};
  }
        ...
        builder.createIntegerConstant(
            ...
            Fortran::runtime::io::HashInquiryKeyword(
                Fortran::parser::InquireSpec::CharVar::EnumToString(
                    std::get<Fortran::parser::InquireSpec::CharVar::Kind>(
                        var.t))
                    .c_str())),
  ...
  mlir::Value genInquireSpec<Fortran::parser::InquireSpec::LogVar>(
    ...
    auto &builder = converter.getFirOpBuilder();
    auto logVarKind =
        std::get<Fortran::parser::InquireSpec::LogVar::Kind>(var.t);
    bool pendId =
        idExpr &&
    ...
    if (pendId)
      args.push_back(
          builder.createConvert(loc, specFuncTy.getInput(1), idExpr));
    else
  ...
  /// If there is an IdExpr in the list of inquire-specs, then lower it and
  /// return the resulting Value. Otherwise, return null.
  static mlir::Value lowerIdExpr(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
  ...
  /// and returning the insertion point as to the initial context. If there are
  /// no inquire-specs, the insertion point is undefined.
  static mlir::OpBuilder::InsertPoint threadInquire(
      Fortran::lower::AbstractConverter & converter, mlir::Location loc,
      mlir::Value cookie, const std::list<Fortran::parser::InquireSpec> &ispecs,
      bool checkResult, mlir::Value &ok) {
    auto &builder = converter.getFirOpBuilder();
    mlir::OpBuilder::InsertPoint insertPt;
