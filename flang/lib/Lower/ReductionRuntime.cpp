...
struct ForcedMaxvalInteger16 {
  static constexpr const char *name = ExpandAndQuoteKey(RTNAME(
                                      MaxvalInteger16));
  static constexpr Fortran::lower::FuncTypeBuilderFunc getTypeModel() {
    return [](mlir::MLIRContext *ctx) {
...
mlir::Value Fortran::lower::genMaxval(Fortran::lower::FirOpBuilder &builder,
  ...
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(1))) 
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MaxvalInteger1)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(2)))
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MaxvalInteger2)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(4)))
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MaxvalInteger4)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(8)))
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MaxvalInteger8)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
...
void Fortran::lower::genMaxvalDim(Fortran::lower::FirOpBuilder &builder,
                                  ...
                                  mlir::Value arrayBox, mlir::Value dim,
                                  mlir::Value maskBox) { 
  auto func = Fortran::lower::getRuntimeFunc<mkRTKey(MaxvalDim)>
                              (loc, builder);
...
void Fortran::lower::genMaxvalChar(Fortran::lower::FirOpBuilder &builder,
                                   mlir::Location loc, mlir::Value resultBox,
                                   mlir::Value arrayBox, mlir::Value maskBox) {
  auto func = Fortran::lower::getRuntimeFunc<mkRTKey(MaxvalCharacter)>
                              (loc, builder);
  auto fTy = func.getType();
  auto sourceFile = Fortran::lower::locationToFilename(builder, loc);
  auto sourceLine =
...
void Fortran::lower::genMinvalDim(Fortran::lower::FirOpBuilder &builder,
                                  ...
                                  mlir::Value arrayBox, mlir::Value dim,
                                  mlir::Value maskBox) { 
  auto func = Fortran::lower::getRuntimeFunc<mkRTKey(MinvalDim)>
                              (loc, builder);
  genReduction3Args(func, builder, loc, resultBox, arrayBox, dim, maskBox);
}
...
void Fortran::lower::genMinvalChar(Fortran::lower::FirOpBuilder &builder,
                                   mlir::Location loc, mlir::Value resultBox,
                                   mlir::Value arrayBox, mlir::Value maskBox) {
  auto func = Fortran::lower::getRuntimeFunc<mkRTKey(MinvalCharacter)>
                              (loc, builder);
  auto fTy = func.getType();
  auto sourceFile = Fortran::lower::locationToFilename(builder, loc);
  auto sourceLine =
...
mlir::Value Fortran::lower::genMinval(Fortran::lower::FirOpBuilder &builder,
  ...
  if (eleTy.isF32())
    ...
    func = Fortran::lower::getRuntimeFunc<ForcedMinvalReal16>(loc, builder);
           ...
           getIntegerBitsize(1))) 
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MinvalInteger1)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(2)))
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MinvalInteger2)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(4)))
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MinvalInteger4)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(8)))
    func = Fortran::lower::getRuntimeFunc<mkRTKey(MinvalInteger8)>(loc, 
                                                                   builder);
  else if (eleTy == builder.getIntegerType(builder.getKindMap().
           getIntegerBitsize(16)))
